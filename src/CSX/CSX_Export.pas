...
const
  ...
  { Divide one big string file into several smaller files.
    This doesn't affect importing, only export uses this option }
  LinesPerFile = 5000;
...
var
  ...
  const
    ...
    Fun5: TFuncBytes = (2, 0, 0, 0, $F4, $66, $17, $7D, 0, 0, 0, 0);
    Fun6: TFuncBytes = (1, 0, 0, 0, $1F, $FF, 0, 0, 0, 0, 0, 0);
    Fun7: TFuncBytes = (2, 0, 0, 0, $2A, $51, $DC, $83, 0, 0, 0, 0);   
  begin                                                          
    Result := AreBytesEqual(@Str[0], @Talk[0], Length(Str)) or
              AreBytesEqual(@Str[0], @Mess[0], Length(Str)) or
              AreBytesEqual(@Str[0], @FAs[0],  8) or
              AreBytesEqual(@Str[0], @FCS[0],  10) or
              AreBytesEqual(@Str[0], @Fun5[0], 8) or
              AreBytesEqual(@Str[0], @Fun6[0], 6) or
              AreBytesEqual(@Str[0], @Fun7[0], 8) 
  end;
  ...
  function GetStrOffset: ShortInt;
  var
    FuncName: PChar;
  begin
    ...
    if (StrLComp(FuncName, 'AS', 2) = 0) or (StrLComp(FuncName, 'CS'#$1C'Y', 4) = 0) then
      Result := 1
      else if Char(Func[4]) in [#$F4, #$2A] then
        Result := -1
        else if Char(Func[4]) in [#$1F] then
          Result := -3
          else
            Result := 4;
  end;
  ...
  begin
    ...
    if (Func[0] = 2) or UnicodeBackwards then
    begin
...
//  while Arc.Position < 02303885 do
  begin
    ...
    Arc.Read(Ch, 1);
    if Ch in ['T', 'M', 'A', 'C', #$F4, #$1F, #$2A, #0] then
    begin
      Arc.Seek(-5, soFromCurrent);
